...
// Get layout from template args (passed via get_template_part)
// When get_template_part is called with array, variables are extracted into scope
// We can access via $args array or directly as $layout (if extracted)
...
// Add class if custom_location_attributes is set
$product_classes = 'woocommerce-loop-product layout-' . esc_attr($layout);
...
if (!empty($custom_location_attributes) && is_array($custom_location_attributes) && $product && $product->is_type('variable')) {
	// Check if product actually has any of these custom attributes
	...
	foreach ($custom_location_attributes as $attr_slug) {
		...
		if (isset($product_attributes[$attr_slug]) || isset($product_attributes[$attribute_slug])) {
	...
	if ($has_custom_attributes) {
		$product_classes .= ' bt-has-custom-attributes';
	}
}
?>
<div <?php wc_product_class($product_classes, $product); ?>>
	<div class="woocommerce-loop-product__thumbnail">
		...
		<?php if ($layout === 'style-6') : ?>
			<div class="bt-product-quick-view">
				<a class="bt-icon-btn bt-product-quick-view-btn" href="#" data-id="<?php echo get_the_ID(); ?>">
					<?php echo esc_html__('Quick View', 'woozio'); ?>
				</a>
			</div>
		<?php endif; ?>
