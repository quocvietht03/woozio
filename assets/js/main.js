!(function ($) {
	...
	jQuery(window).on('elementor/popup/show', function (event, id, instance) {
		...
		// Fix keyboard focus issue on mobile when typing in search input
		var $searchInput = $searchWidget.find('.bt-live-search, .bt-keyword-suggest').first();
		var isMobile = jQuery(window).width() <= 570;
		
		if ($searchInput.length && isMobile) {
			var isTyping = false;
			var typingTimer = null;
			var lastInputValue = '';
			var activeInput = $searchInput[0];
			...
			$searchInput.on('input focus', function() {
				...
				// Reset typing flag after user stops typing for 2 seconds
				typingTimer = setTimeout(function() {
					isTyping = false;
					// Keep shouldMaintainFocus true for a bit longer
					setTimeout(function() {
			...
			// Use focusout instead of blur for better control
			$searchInput.on('focusout', function(e) {
				if (shouldMaintainFocus && lastInputValue.length >= 2) {
					var inputElement = this;
					var relatedTarget = e.relatedTarget;
					
					// Don't prevent if focusing to another element in the search widget
					if (relatedTarget && jQuery(relatedTarget).closest('.bt-elwg-search-product-style-1').length) {
						return;
					}
					
					// Prevent the focusout
					e.preventDefault();
					e.stopPropagation();
					
					// Restore focus immediately and aggressively
					setTimeout(function() {
						inputElement.focus();
					}, 0);
			...
			// Hook into document level to intercept ALL focus changes
			var documentFocusHandler = function(e) {
				if (shouldMaintainFocus && lastInputValue.length >= 2 && activeInput) {
					var target = e.target;
					
					// Allow focus within search widget
					if (target && jQuery(target).closest('.bt-elwg-search-product-style-1').length) {
						return;
					}
					
					// Allow focus on close button
					if (target && jQuery(target).closest('.elementor-popup-modal__close').length) {
						shouldMaintainFocus = false;
						return;
					}
					
					// If focus is moving away from our input, prevent it
					if (document.activeElement !== activeInput && target !== activeInput) {
						e.preventDefault();
						e.stopPropagation();
			...
			// Use capture phase on document to intercept early
			document.addEventListener('focusin', documentFocusHandler, true);
			document.addEventListener('mousedown', documentFocusHandler, true);
			document.addEventListener('touchstart', documentFocusHandler, true);
			
			// Also prevent on popup level
			var popupElement = $popup[0];
			if (popupElement) {
				var handlePopupInteraction = function(e) {
					if (shouldMaintainFocus && lastInputValue.length >= 2) {
						var $target = jQuery(e.target);
						
						// Allow interactions within search widget
						if ($target.closest('.bt-elwg-search-product-style-1').length) {
							return;
						}
						
						// Allow close button
						if ($target.closest('.elementor-popup-modal__close, .elementor-popup-modal__overlay').length) {
							shouldMaintainFocus = false;
							return;
						}
				...
				popupElement.addEventListener('mousedown', handlePopupInteraction, true);
				popupElement.addEventListener('touchstart', handlePopupInteraction, true);
				popupElement.addEventListener('focusin', handlePopupInteraction, true);
				
				$popup.data('focus-handlers', {
					mousedown: handlePopupInteraction,
					touchstart: handlePopupInteraction,
					focusin: handlePopupInteraction
				});
			}
			
			// Very aggressive focus monitoring
			...
			var cleanupHandler = function(e, popupId) {
				if (popupId === id) {
					...
					$searchInput.off('input focus focusout');
					
					// Remove document listeners
					document.removeEventListener('focusin', documentFocusHandler, true);
					document.removeEventListener('mousedown', documentFocusHandler, true);
					document.removeEventListener('touchstart', documentFocusHandler, true);
					
					// Remove popup listeners
					var handlers = $popup.data('focus-handlers');
					if (handlers && popupElement) {
						popupElement.removeEventListener('mousedown', handlers.mousedown, true);
			...
			jQuery(window).on('elementor/popup/hide', cleanupHandler);
		}
