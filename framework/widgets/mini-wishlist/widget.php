...
class Widget_MiniWishlist extends Widget_Base
{
	...
	protected function render()
	{
		...
		$wishlist_count = count(WC()->session->get('productwishlistlocal', []));
		$wishlist_empty_class = ($wishlist_count === 0) ? 'wishlist-empty' : '';
?>
		<div class="bt-elwg-mini-wishlist--default">
			<div class="bt-mini-wishlist <?php echo esc_attr($wishlist_empty_class); ?>">
				<a class="bt-toggle-btn" href="<?php echo esc_url($wishlist_url); ?>">
					<?php if (!empty($icon_wishlist) && 'svg' === pathinfo($icon_wishlist, PATHINFO_EXTENSION)) {
						$response = wp_safe_remote_get( $icon_wishlist, array(
							'timeout' => 20,
							'headers' => array(
					...
					<?php } ?>
					<span class="wishlist_total"><?php echo esc_html($wishlist_count); ?></span></a>
